import { MessageSquare, Upload, BarChart3, Box, ShieldCheck, Zap } from "lucide-react";
...
export default function Sidebar() {
    ...
    return (
      <Link
        ...
        className={`group relative flex items-center gap-3 w-full px-4 py-3 rounded-xl transition-all duration-300 ${
          isActive
            ? "bg-blue-600/10 text-blue-400 shadow-[inset_0_0_12px_rgba(37,99,235,0.2)]"
  ...
  return (
    <aside className="w-72 bg-slate-950/40 backdrop-blur-2xl border-r border-white/5 flex flex-col p-6 overflow-hidden">
      ...
      <div className="flex items-center gap-3 mb-12 px-2">
        ...
        <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em]">Intelligence</p>
        </div>
      </div>

      {/* Navigation Group */}
      <div className="space-y-1">
        <p className="px-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4">Core Systems</p>
      
          <nav className="space-y-1.5">
            {/* path "/" maps to ChatPage in your App.jsx */}
            {navItem("/", <MessageSquare size={19} />, "Neural Chat")}
            
            {/* path "/upload" maps to UploadPage in your App.jsx */}
            {navItem("/upload", <Upload size={19} />, "Data Uplink")}
            
            {/* path "/analytics" maps to AnalyticsPage in your App.jsx */}
      ...
      <div className="mt-auto pt-6 border-t border-white/5">
        <div className="bg-slate-900/50 rounded-xl p-4 border border-white/5">
            <div className="flex items-center gap-3">
                ...
                <span className="text-[11px] font-mono text-slate-400">CORE_LINK_ESTABLISHED</span>
            </div>

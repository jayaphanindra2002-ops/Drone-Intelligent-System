...
export default function DroneBackground() {
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0 transition-opacity duration-1000">
      ...
      <div className="absolute top-[20%] left-[-10%] animate-droneFlight opacity-10 dark:opacity-20 transition-all duration-500">
      ...
      <div 
        className="absolute top-[50%] left-[-20%] animate-droneFlight opacity-[0.03] dark:opacity-10 transition-all duration-500"
        style={{ animationDelay: '5s', animationDuration: '35s' }}
      >
        <Plane className="w-8 h-8 text-slate-500 dark:text-indigo-300 rotate-90" />
      </div>

      {/* Tertiary Drone - Far off in the distance */}
      <div 
        className="absolute top-[10%] left-[-30%] animate-droneFlight opacity-[0.02] dark:opacity-5 transition-all duration-500"
        style={{ animationDelay: '12s', animationDuration: '45s' }}
      >
        <Plane className="w-6 h-6 text-slate-600 dark:text-blue-200 rotate-90" />
      </div>
